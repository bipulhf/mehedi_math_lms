...
```bash
sudo mkdir -p /opt/genex
sudo chown "$USER":"$USER" /opt/genex
git clone -b genex https://github.com/bipulhf/mehedi_math_lms.git /opt/genex
cd /opt/genex
...
`/etc/systemd/system/genex.service`:

```ini
[Unit]
Description=Genex (Docker Compose)
...
```bash
sudo systemctl enable --now genex.service
```
...
# with psql.
docker compose exec postgres pg_dump -U postgres genex | gzip > "backup-$(date +%F).sql.gz"
```
...
sudo mkdir -p /opt/genex
sudo chown deploy:deploy /opt/genex
```
...
sudo -u postgres psql -c "CREATE USER genex WITH PASSWORD 'change-me';"
sudo -u postgres psql -c "CREATE DATABASE genex OWNER genex;"
```
...
`EnvironmentFile=/opt/genex/.env`

**`/etc/systemd/system/genex-api.service`**

```ini
[Unit]
Description=Genex — API
After=network.target postgresql.service

[Service]
Type=simple
User=deploy
WorkingDirectory=/opt/genex/apps/api
EnvironmentFile=/opt/genex/.env
...
**`/etc/systemd/system/genex-web.service`**

```ini
[Unit]
Description=Genex — Web
After=network.target genex-api.service
Requires=genex-api.service

[Service]
Type=simple
User=deploy
WorkingDirectory=/opt/genex/apps/web
EnvironmentFile=/opt/genex/.env
...
| Unit file | `ExecStart` |
|---|---|
| `genex-worker-notification.service` | `.../bun src/workers/notification-worker.ts` |
| `genex-worker-sms.service` | `.../bun src/workers/sms-worker.ts` |
| `genex-worker-file-processing.service` | `.../bun src/workers/file-processing-worker.ts` |
| `genex-worker-audit-log-cleanup.service` | `.../bun src/workers/audit-log-cleanup-worker.ts` |

(`.../bun` = `/home/deploy/.bun/bin/bun`; `WorkingDirectory` =
`/opt/genex/apps/api` for all four, same `User`/
`EnvironmentFile`/`Restart` lines as `genex-api.service` above.)

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now genex-api.service genex-web.service
# and the four worker units, if using Redis
sudo systemctl status genex-api.service
curl -s http://localhost:3001/api/health | head -c 300
```
...
bun run db:migrate
exit
sudo systemctl restart genex-api.service genex-web.service
...
```bash
sudo -u postgres pg_dump genex | gzip > "backup-$(date +%F).sql.gz"
```
...
- If Redis is enabled, confirm the four workers are actually running
  (`docker compose ps` / `systemctl status genex-worker-*`) — a background
