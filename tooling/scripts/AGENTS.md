...
- Env comes from the root `.env` via `bun --env-file ../../.env`. Validate any variable you read with a Zod schema at the top of the file, as `seed.ts` does. `@mma/db` throws at import time without `DATABASE_URL`, so the env file is not optional.
...
Do not extend the local copy. If you need slug behaviour here, switch the import to `@mma/shared` (already a dependency of this workspace) and delete `slug.ts` as its own change.
