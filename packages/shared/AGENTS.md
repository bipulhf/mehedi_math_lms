...
src/mcq-import.ts     wrapBareLatex, parseMcqImport — reading a paper out of the Bijoy-to-LaTeX converter.
```
...
- **MCQ import.** The owner's Bijoy-to-LaTeX converter emits bare LaTeX concatenated into the question string with no delimiters, so the importer has to decide which spans are maths. One rule: a run becomes maths only when it holds an unmistakable LaTeX control sequence, and it stops at the first Bengali character — under-wrapping shows the teacher LaTeX they can fix, over-wrapping renders their prose as italic variables and nobody notices. It lives here rather than in `apps/web` because it is pure, it is the part worth testing against a real export, and it is the only place that rule may be written. See [ADR-0018](../../docs/adr/0018-an-imported-question-is-a-draft-until-somebody-answers-it.md).
