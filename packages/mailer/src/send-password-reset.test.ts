...
function requestWith(cookie: string | null): Request {
  return new Request("https://genex.com.bd/api/auth/request-password-reset", {
...
describe("localeFromRequest", () => {
  test("follows the reader's choice", () => {
    expect(localeFromRequest(requestWith("genex_locale=en"))).toBe("en");
    expect(localeFromRequest(requestWith("genex_locale=bn"))).toBe("bn");
  });

  test("finds the cookie beside others", () => {
    expect(localeFromRequest(requestWith("theme=dark; genex_locale=en; ga=1"))).toBe("en");
  });
  ...
  test("falls back to Bangla with no cookie, no request, or a nonsense value", () => {
    ...
    expect(localeFromRequest(requestWith("genex_locale=fr"))).toBe("bn");
  });
});
