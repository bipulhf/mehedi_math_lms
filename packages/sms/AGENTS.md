...
- **A failed send throws.** Same reasoning as `@mma/mailer`: a person waiting for a code that was never sent, with nothing in the log saying so, is the failure this package exists to avoid. The bulk path is the exception — it records `FAILED` per recipient because an admin needs the list, not an exception.
