    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <Skeleton key={i} className="h-96 w-full bg-surface-container-low/50 rounded-4xl border border-outline-variant/10 shadow-sm" />
        ))}
      </div>
    </div>
  );
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
          <div className="space-y-3">
             <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="font-headline text-3xl font-extrabold tracking-tight text-on-surface">Academy Workshop</h3>
             </div>
             <p className="text-sm text-on-surface-variant font-light max-w-2xl leading-relaxed">
               Craft and curate your academic offerings. Build drafts, refine content, and monitor the journey from proposal to publication.
             </p>
          </div>

          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
             <div className="min-w-48 space-y-2">
                <Label htmlFor="status-filter" className="text-[0.65rem] font-bold uppercase tracking-widest text-on-surface/40 ml-1">Status Filter</Label>
             ...
             <div className="sm:pt-6">
                <Button asChild className="h-12 rounded-2xl px-8 font-headline font-extrabold shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98] w-full">
      ...
      {courses.length === 0 ? (
        <div className="rounded-4xl bg-surface-container-lowest/40 border border-dashed border-outline-variant/20 py-24 text-center group transition-all hover:bg-surface-container-lowest/60">
          ...
          <p className="font-headline text-2xl font-extrabold text-on-surface mb-2 tracking-tight">Empty Curriculum</p>
          <p className="text-sm text-on-surface-variant max-w-xs mx-auto font-light leading-relaxed italic">
            No courses in this category. Propose your first draft and start your instructional journey.
          </p>
        </div>
      ) : (
        <div className="grid gap-8 lg:grid-cols-2">
          {courses.map((course) => (
            <div key={course.id} className="group/card flex flex-col rounded-4xl border border-outline-variant/40 bg-surface-container-lowest/80 backdrop-blur-3xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-2xl hover:border-primary/20 relative">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 border-t border-outline-variant/10 bg-primary/2 p-6 relative z-10">
                <div className="flex flex-wrap items-center gap-4">
                  <CourseStatusBadge status={course.status} />
                  <div className="flex items-center gap-2 text-[0.65rem] font-bold uppercase tracking-widest text-on-surface/40">
