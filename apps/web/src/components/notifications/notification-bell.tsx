  ...
  return (
    ...
    <div className="static sm:relative" ref={panelRef}>
      ...
      >
        ...
        {unread > 0 ? (
          <span className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-accent px-1 text-[0.6rem] font-bold leading-4 text-on-accent">
            {unread > 99 ? "99+" : unread}
          </span>
        ) : null}
      </button>
      {open ? (
        <div
          className={cn(
            "absolute inset-x-3 top-full z-50 mt-2 sm:inset-x-auto sm:right-0 sm:top-auto sm:mt-3 sm:w-96",
            "rounded-[var(--radius-md)] border border-hairline bg-popover p-3"
          )}
